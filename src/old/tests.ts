...
import { encode, decode, createDefaultDict } from "../encoder/encoder";
import { initProtocols, protoDecode, protoEncode } from "./protocol";

import PROTOCOL_TEST from "../test-data/protocol-test";
...
function test() {
    ...
    let protocols = {
        ...
        "update": { schema: 2, protocol: PROTOCOL_TEST }
    }

    createDefaultDict(defaultDictionary);
    initProtocols(protocols);
    ...
    let encoded = protoEncode(simpleTest);
...
function testObjectDeltaPlus() {
    ...
    console.time("Encoding");
    let encoded = protoEncode(update);
    console.timeEnd("Encoding");
    console.time("Decoding");
    let decoded = protoDecode(encoded);
    console.timeEnd("Decoding");
...
function runAllTests() {
    const protocols = { "update": { schema: 2, protocol: PROTOCOL_TEST } };
    createDefaultDict(defaultDictionary);
    initProtocols(protocols);

    type TC = { name: string; payload?: any; from?: any; to?: any };
    const cases: TC[] = [
    ...
    for (const tc of cases) {
        ...
        if (tc.from && tc.to) payload = delta(tc.from, tc.to);
        const msg = { type: "update", payload };
        try {
            const encoded = protoEncode(msg);
...
function runEncodingTest() {
    ...
    createDefaultDict(defaultDictionary);
    initProtocols({ "update": { schema: 2, protocol: PROTOCOL_TEST } });
