...
import { createDefaultDict, decode, deserialize, deserializeEX, encode, serialize, serializeEX } from "../encoder/encoder";
 ...
 */
export function encodeLEB128(buffer, value: number): number[] {
 ...
 */
export function decodeLEB128(bytes: ArrayLike<number>, offset = 0): { value: number; bytesRead: number } {
    let value = 0;
    let shift = 0;
    let bytesRead = 0;
    while (true) {
        const byte = (bytes as any)[offset + bytesRead];
        if (byte === undefined) throw new RangeError('decodeLEB128: unexpected end of input');
        bytesRead++;
 ...
 */
export function encodeSLEB128(buffer, value: number): number[] {
    if (!Number.isInteger(value)) throw new RangeError('encodeSLEB128 requires an integer');
    let more = true;
    while (more) {
        let byte = value & 0x7F;
 ...
 * Returns the decoded value and the number of bytes consumed.
 */
export function decodeSLEB128(bytes: ArrayLike<number>, offset = 0): { value: number; bytesRead: number } {
    let value = 0;
    let shift = 0;
    let bytesRead = 0;
    let byte: number;
    do {
        byte = (bytes as any)[offset + bytesRead];
        if (byte === undefined) throw new RangeError('decodeSLEB128: unexpected end of input');
        bytesRead++;
