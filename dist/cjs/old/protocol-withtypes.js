...
exports.setStringDictionaryWT = setStringDictionaryWT;
exports.compileSchema = compileSchema;
exports.encodeNode = encodeNode;
exports.decodeNode = decodeNode;
exports.registerProtocolWT = registerProtocolWT;
exports.protoEncodeWT = protoEncodeWT;
exports.protoDecodeWT = protoDecodeWT;
...
const _emptyDict = { count: 0, keys: {}, order: [], frozen: true };
// ─── String dictionary ────────────────────────────────────────────────────────
// Up to 255 entries.  Index 0-126 encodes as a single byte; 127-254 as 2 bytes.
let _stringDictionary = [];
let _stringDictionaryLookup = new Map();
/**
 ...
 */
function setStringDictionaryWT(dict) {
...
// ─── LEB128 helpers ───────────────────────────────────────────────────────────
function writeLEB128(buffer, value) {
...
function writeSLEB128(buffer, value) {
...
function compileSchema(schema) {
    ...
    if ('$map' in schema) {
        ...
        const valueNode = (inner && Object.keys(inner).length > 0)
            ? compileSchema(inner)
 ...
 */
function encodeStringWT(value, buffer) {
    if (value === '') {
        buffer.push(0x00);
        return;
    }
...
function encodeNode(value, node, buffer) {
    switch (node.kind) {
...
function encodePrimitive(value, type, buffer) {
    switch (type) {
        case 'uint':
        ...
        default:
            ...
            (0, encoder_1.serializeEX)(value, buffer, _emptyDict, {});
            return;
    }
}
/**
 ...
 */
function encodeObject(value, fields, buffer) {
    if (fields.length === 0) {
 ...
 */
function encodeMap(value, valueNode, buffer) {
 ...
 */
function encodeArray(value, elementNode, buffer) {
    ...
    if (isDelta) {
        ...
        for (const op of value)
            encodeArrayOp(op, elementNode, buffer);
    }
    else {
        buffer.push(0); // mode 0: standard
        writeLEB128(buffer, value.length);
        for (const item of value)
            encodeNode(item, elementNode, buffer);
    }
}
...
function encodeArrayOp(op, elementNode, buffer) {
    switch (op.op) {
        case 'resize':
        ...
        case 'setrange':
            ...
            for (const v of op.values)
                encodeNode(v, elementNode, buffer);
            return;
        case 'fill':
 ...
 */
function encodeStaticArray(value, elementNode, buffer) {
    if (!Array.isArray(value)) {
...
function decodePrimitive(ref, type) {
    switch (type) {
 ...
 */
function decodeObject(ref, fields) {
    const result = {};
 ...
 */
function decodeMap(ref, valueNode) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        for (let j = 0; j < keyLen; j++)
            keyArr[j] = ref.view.getUint8(ref.pos++);
 ...
 */
function decodeArray(ref, elementNode) {
    ...
    if (mode === 0) {
        ...
        const result = [];
        for (let i = 0; i < count; i++)
            result.push(decodeNode(ref, elementNode));
        return result;
    }
    // Mode 1: flexible ops
    const result = [];
    for (let i = 0; i < count; i++)
        result.push(decodeArrayOp(ref, elementNode));
    return result;
}
function decodeArrayOp(ref, elementNode) {
    ...
    switch (opCode) {
        ...
        case AROP_SETRANGE: {
            ...
            const values = [];
            for (let i = 0; i < n; i++)
                values.push(decodeNode(ref, elementNode));
 ...
 */
function decodeStaticArray(ref, elementNode) {
    const mode = ref.view.getUint8(ref.pos++);
    switch (mode) {
        case 2: {
            const count = readLEB128(ref);
...
const _typeIndexToName = new Map();
...
function registerProtocolWT(protocolOrTypeName, payloadSchema) {
    let typeName;
    let schema;
    ...
    if (!typeName)
        throw new Error('[protocol-withtypes] Protocol must have a "type" string field');
    if (_protocols.has(typeName))
        return;
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function protoEncodeWT(data) {
    const proto = _protocols.get(data?.type);
    const buffer = [];
    if (!proto) {
        buffer.push(0); // typeIndex 0 = unknown / fallback
        const dict = (0, encoder_1.createDefaultDict)();
        dict.frozen = true;
        (0, encoder_1.serializeEX)(data, buffer, dict, {});
...
//# sourceMappingURL=protocol-withtypes.js.map