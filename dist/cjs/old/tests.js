...
function areObjectsEqual(obj1, obj2) {
    ...
    if (typeof obj1 !== "object")
        return obj1 === obj2;
    var arr1 = Array.isArray(obj1);
    var arr2 = Array.isArray(obj2);
    if (arr1 !== arr2)
        return false;
...
function test() {
    ...
    var encoded = (0, protocol_1.protoEncode)(simpleTest);
    console.timeEnd("Encoding");
    console.time("Decoding");
    var decoded = (0, protocol_1.protoDecode)(encoded);
...
function testObjectDeltaPlus() {
    ...
    var diff = (0, delta_1.delta)(from, to);
    ...
    console.time("Encoding");
    var encoded = (0, protocol_1.protoEncode)(update);
    console.timeEnd("Encoding");
    console.time("Decoding");
    var decoded = (0, protocol_1.protoDecode)(encoded);
    console.timeEnd("Decoding");
...
function runAllTests() {
    var protocols = { "update": { schema: 2, protocol: protocol_test_1.default } };
    (0, encoder_1.createDefaultDict)(defaultDictionary);
    (0, protocol_1.initProtocols)(protocols);
    var cases = [
    ...
    for (var _i = 0, cases_1 = cases; _i < cases_1.length; _i++) {
        var tc = cases_1[_i];
        var payload = tc.payload;
        if (tc.from && tc.to)
            payload = (0, delta_1.delta)(tc.from, tc.to);
        var msg = { type: "update", payload: payload };
        try {
            var encoded = (0, protocol_1.protoEncode)(msg);
            var decoded = (0, protocol_1.protoDecode)(encoded);
            var ok = areObjectsEqual(msg, decoded);
            if (ok) {
                pass++;
                console.log("  PASS  [".concat(encoded.byteLength, "B]  ").concat(tc.name));
            }
            else {
                fail++;
                console.log("  FAIL        ".concat(tc.name));
                console.log("    orig:    ".concat(JSON.stringify(msg)));
                console.log("    decoded: ".concat(JSON.stringify(decoded)));
            }
        }
        catch (e) {
            fail++;
            console.log("  ERROR       ".concat(tc.name, ": ").concat(e));
        }
    }
    console.log("\n".concat(pass, " passed, ").concat(fail, " failed  (").concat(pass + fail, " total)"));
}
function runEncodingTest() {
    ...
    for (var i = 0; i < tests.length; i++) {
        var test_1 = tests[i];
        console.time("Encoding " + i);
        var encoded = (0, protocol_1.protoEncode)(test_1);
        console.timeEnd("Encoding " + i);
        console.time("Decoding " + i);
        var decoded = (0, protocol_1.protoDecode)(encoded);
        console.timeEnd("Decoding " + i);
        console.log("Encoded (original size):", JSON.stringify(test_1).length, "bytes");
        console.log("Encoded (size):", encoded.byteLength, "bytes");
        console.log("Original:", JSON.stringify(test_1));
        console.log("Decoded: ", JSON.stringify(decoded));
    }
}
// runAllTests();
