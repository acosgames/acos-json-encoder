...
function protoEncode(data, dictionary = undefined) {
    if (!data?.type) {
    ...
    // }
    let dict = (0, encoder_1.createDefaultDict)(dictionary);
    dict.frozen = true;
    let buffer = [];
    let schemaId = 0;
    ...
    let payload = (0, encoder_1.serializeEX)(data, buffer, dict, {});
    return new Uint8Array(buffer).buffer;
    // return ENCODER.encode(data);
}
function protoDecode(data, dictionary = undefined) {
    let dict = (0, encoder_1.createDefaultDict)(dictionary);
    dict.frozen = true;
