...
function pushSet(changes, index, value) {
 ...
 */
export function arrayDelta(from, to) {
    const changes = [];
 ...
 */
export function merge(from, delta) {
    if (Array.isArray(from) && Array.isArray(delta)) {
        return arrayMerge(from, delta);
    }
 ...
 * `from` is not mutated.
 */
export function arrayMerge(from, changes) {
    const result = [...from];
...
function objectMerge(from, delta) {
    const result = { ...from };
    for (const key of Object.keys(delta)) {
        if (key === '$deleted') {
            for (const k of delta[key])
                delete result[k];
            continue;
        }
        const d = delta[key];
 ...
 */
export function hidden(obj) {
    if (Array.isArray(obj)) {
        const result = new Array(obj.length).fill(null);
        let hasHidden = false;
 ...
 */
export function unhidden(obj, extracted) {
    ...
    if (isPlainObject(obj) && isPlainObject(extracted)) {
        for (const key of Object.keys(extracted)) {
            if (key[0] === '_') {
                ...
                unhidden(obj[key], extracted[key]);
            }
        }
    }
}
