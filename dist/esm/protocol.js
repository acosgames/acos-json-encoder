import { createDefaultDict, deserializeEX, serializeEX } from "./encoder";
...
function precomputeProtocol(protocol) {
    ...
    for (const key in protocol) {
        ...
        const byte = (typeof entry === 'number') ? entry : entry?.$byte;
        if (byte === undefined)
            continue;
    ...
    protocol.$schemaKeySet = new Set(sorted.map(e => e.lookupKey));
    // Recurse into nested sub-protocols
    for (const e of sorted) {
        if (e.protoEntry?.$object)
            precomputeProtocol(e.protoEntry.$object);
        if (e.protoEntry?.$array)
...
export function initProtocols(_messageProtocols) {
...
function recurseProtocolEncode(payload, buffer, dictionary, protocol, cache = {}) {
    ...
    const sortedSchema = protocol.$sortedSchema;
...
export function protoEncode(data, dictionary = undefined) {
    if (!data?.type) {
...
export function protoDecode(data, dictionary = undefined) {
    let dict = createDefaultDict(dictionary);
    dict.frozen = true;
