...
let registeredProtocols = [{ type: 'default', index: 0, schema: compileSchema({}) }]; // 1-based indexing; position 0 is reserved for fallback
let protocolMap = {};
export function setDefaultDictionary(dictionaryList) {
 ...
 *   });
 */
export function revertProtocol(baseType) {
...
export function extendProtocol(baseType, overrides) {
    const protocol = getProtocol(baseType);
...
export function getProtocolSchema(type) {
    const protocol = getProtocol(type);
    if (!protocol || protocol.index === 0)
        return null;
    return describeNode(protocol.schema);
}
export function registerProtocol(protocol, dictionaryList) {
...
export function protoEncode(payload) {
...
export function encodeNode(value, protocol, node, buffer, cache = {}) {
    switch (node.kind) {
...
export function protoDecode(data, dictionaryList) {
    let view;
...
// ─── Decoder ──────────────────────────────────────────────────────────────────
export function decodeNode(ref, protocol, node) {
    switch (node.kind) {
        case 'any': {
