...
let indexToMessageType = [];
let messageProtocolByteMap = {};
...
function initProtocols(_messageTypes, _messageProtocols) {
    ...
    for (let [messageType, protocol] of Object.entries(messageProtocols)) {
        let byteMap = {};
        messageProtocolByteMap[messageType] = mapByteToProtocol(protocol, byteMap);
    }
}
        ...
        if (key[0] === '#') {
            ...
            if (typeof protoEntry === 'object') {
                if (protoEntry.$object) {
                    ...
                    (0, encoder_1.serializeEX)(payload[key], buffer, dictionary, cache);
                }
            }
            else {
                (0, encoder_1.serializeEX)(payload[key], buffer, dictionary, cache);
    ...
    for (let key in payload) {
        if (key[0] === '#') {
            if (!protocol[key.substring(1)]?.$array) {
                (0, encoder_1.serializeEX)(key, buffer, dictionary, cache);
                (0, encoder_1.serializeEX)(payload[key], buffer, dictionary, cache);
            }
        }
        else if (protocol[key] === undefined) {
...
function protocolEncode(buffer, payload, dictionary, protocol) {
    let cache = {};
    ...
    // let protocolBuffer = [];
    recurseProtocolEncode(payload, buffer, protocol, cache);
...
function recurseProtocolDecode(ref, json, dictionary, protocol) {
    ...
    for (let i = 0; i < nonSchemaCount; i++) {
        let key = (0, encoder_1.deserializeEX)(ref);
        json[key] = (0, encoder_1.deserializeEX)(ref);
    }
    return json;
}
function protoEncode(data, dictionary, protocol) {
    if (!data?.type) {
    ...
    let buffer = [];
    buffer.push(messageTypes[data.type] || 0); // default to 0 if type is unknown
    if (messageProtocols[data.type]) {
        return protocolEncode(buffer, data.payload, dictionary, messageProtocols[data.type]);
    }
    let payload = (0, encoder_1.encode)(data, dictionary);
    return payload;
...
function protoDecode(data, dictionary) {
    ...
    if (view) {
        ...
        if (typeName && messageProtocols[typeName]) {
            let payload = protocolDecode(view, pos, dictionary, messageProtocolByteMap[typeName]);
            return { type: typeName, payload };
        }
    }
    // Standard path: ENCODER.encode(data) was used, so ENCODER.decode reverses it.
    return (0, encoder_1.deserialize)(view, 0, dictionary);
}
function test() {
    // initProtocols();
    let testData = {
        ...
        payload: {
            state: { cells: ["X", "O", "", "", "X", "", "", "", "O"] },
            players: [
                { displayname: "Alice", shortid: "a1", portraitid: "p1" },
                { displayname: "Bob", shortid: "b2", portraitid: "p2" }
            ],
            teams: [
                { name: "Team X", color: "red", order: 1 },
                { name: "Team O", color: "blue", order: 2 }
            ]
        }
    };
    let testData2 = {
    ...
    for (let i = 0; i < 1; i++) {
        ...
        let encoded2 = acosEncode(testJSON4);
        console.timeEnd("Encoding " + i);
        console.time("Decoding " + i);
        let decoded2 = acosDecode(encoded2);
        console.timeEnd("Decoding " + i);
    }
    console.time("Encoding");
    let encoded = acosEncode(testJSON4);
    console.timeEnd("Encoding");
    console.time("Decoding");
    let decoded = acosDecode(encoded);
    console.timeEnd("Decoding");
    console.log("Encoded (original size):", Buffer.from(JSON.stringify(testJSON4)).length, "bytes");
    console.log("Encoded (size):", Buffer.from(encoded).length, "bytes");
    // console.log("Original:", JSON.stringify(testJSON4));
    // console.log("Decoded: ", JSON.stringify(decoded));
    console.log("Is Match:", JSON.stringify(testJSON4) === JSON.stringify(decoded));
}
// test();
